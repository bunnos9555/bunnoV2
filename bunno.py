...
firebase_credentials = {  # Replace with your own Firebase credentials . FORMAT: { "type": "service_account", "project_id": "project-id", "private_key_id": "private-key-id", "private_key": "private-key   
    
    
    
    
    
    }              #UR firebase_credentials.json [IF YOU DONT KNOW HOW TO GET THIS FILE, DM ME ON TELEGRAM @TancantDac]
...
ADMIN_ID = 1055000829  # Replace with the actual admin's user ID
...
def run_attack(target, port, attack_time, sent_message):
    try:
        full_command = f"./bunno {target} {port} {attack_time}"
...
def handle_contact_admin(message):
    bot.reply_to(message, f"📞 @BUNNO_SELLING: @TancantDac")
